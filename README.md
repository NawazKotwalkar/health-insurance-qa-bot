...
This project demonstrates practical GenAI and LLM engineering skills, specifically around document ingestion, text embedding, semantic vector search, and LLM context-window management.
...
4. **User Interface (`app/streamlit_app.py`)**: A Streamlit chat UI that wraps the retrieval and generation pipeline into an interactive chatbot experience.
...
3. **Configure Environment:**
   Copy `.env.example` to a new `.env` file and insert your Gemini API Key.
   ```bash
   cp .env.example .env
   ```
